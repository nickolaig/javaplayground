...
public class JpaUserService implements UserService {
	...
	public UserEntity addNewUser(UserEntity user) throws Exception {
		UserKey userEmailPlaygroundKey = new UserKey(user.getUserEmailPlaygroundKey().getEmail(), user.getUserEmailPlaygroundKey().getPlayground());
		if (this.users.existsById(userEmailPlaygroundKey)) {
			throw new UserAlreadyExistsException("User exists with email: " + userEmailPlaygroundKey.getEmail() + " in playground: "
					+ userEmailPlaygroundKey.getPlayground());
		}
...
//		this.idGenerator.delete(tmp);
	...
	public UserEntity getUserByEmailAndPlayground(String email, String playground) throws Exception {
		...
		return this.users.findById(new UserKey(email, playground))
	...
	public void updateUser(UserKey userEmailPlaygroundKey, UserEntity entityUpdates) throws Exception {
		...
		UserEntity existing = this.users.findById(userEmailPlaygroundKey).orElseThrow(() -> new UserNotFoundException("No such user with email: " + userEmailPlaygroundKey.getEmail() + " and playground: " + userEmailPlaygroundKey.getPlayground()));
